...
static void place(void *bp, size_t asize)
{
    ...
    {
        PUT(HDRP(bp),PACK(asize,GET_PREV_ALLOC(bp),1));//The first block is allocated
        bp=NEXT_BLKP(bp);
        PUT(HDRP(bp),PACK(rest,GET_PREV_ALLOC(bp),0));//The next block is not allocated and the previous block of the current block is allocated
        PUT(FTRP(bp),PACK(rest,GET_PREV_ALLOC(bp),0));//tail 
        add_to_free_list(bp);
        /* ??? */
    }
    else
    {
        PUT(HDRP(bp),PACK(total_size,GET_PREV_ALLOC(bp),1));
        bp=NEXT_BLKP(bp);
        if(GET_ALLOC(HDRP(bp))){//modify the next_block infomation if it has been allocated
            PUT(HDRP(bp),PACK(GET_SIZE(HDRP(bp)), 1, 1));
        }
        else{//not allocated
            PUT(HDRP(bp),PACK(GET_SIZE(HDRP(bp)),1,0));
            PUT(FTRP(bp),PACK(GET_SIZE(FTRP(bp)),1,0));
        }
        /* ??? */
    }
}
